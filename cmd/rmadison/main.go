...
type PackageInfo struct {
	...
	SuitePocket string `json:"pocket"`
}
...
func getIndex(client *resty.Client, pkgInfoChan chan *PackageInfo, series, pocket, component, arch string) error {
		...
		for _, line := range infoLines {
			...
			if key == "Package" {
				...
				pkgInfo = &PackageInfo{
					Name:        pkgName,
					Component:   component,
					SuitePocket: fmt.Sprintf("%v%v", series, pocket),
				}
			}
			if key == "Version" {
...
func (c *Cache) updateCache() {
	...
	resultChan := make(chan *PackageInfo, 1000)
	...
	for {
		...
		case info := <-resultChan:
			if _, exist := c.Packages[info.Name]; !exist {
				c.Packages[info.Name] = []*PackageInfo{info}
				continue
			}

			updated := false
			for _, currentInfo := range c.Packages[info.Name] {
				if currentInfo.Component == info.Component && currentInfo.SuitePocket == info.SuitePocket {
